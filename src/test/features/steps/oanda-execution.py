...
def step_impl(context, trade):
	assert not (('code' in context.response) or ('message' in context.response)), 'Unexpected Oanda response, code :(%s), message : "%s"' % (context.response['code'], context.response['message'])
...
def step_impl(context):
	...
	for order in context.response['orders']:
		if order['side'] == oe.side and order['units'] == oe.units and order['instrument'] == oe.instrument and order['type'] == oe.order_type and 'expiry' in order and order['expiry'] == oe.expiry and 'price' in order and order['price'] == oe.price:
			found = True
			break
	assert found
