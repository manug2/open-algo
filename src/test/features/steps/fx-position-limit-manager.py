...
def step_impl(context, field, value):
    ...
    assert float(expected) == float(observed), 'value of [%s] in filtered order is wrongly [%s], expected=[%s]' % (
...
def step_impl(context, field):
    ...
    observed = getattr(context.filteredOrder, field)
    assert float(expected) == float(observed), 'value of [%s] in filtered order is wrong [%s], expected=[%s]' % (
    field, observed, expected)
...
def step_impl(context, field, currValue):
    ...
    observed = getattr(context.filteredOrder, field)
    assert expected == float(observed), 'value of [%s] in filtered order is wrong [%s], expected=[%s]' % (
    field, observed, expected)
