...
class CcyExposureLimitRiskEvaluator(RiskManager):
	...
	def __init__(self, baseCurrency
		...
		assert ccyLimitShort <= 0, '[%s] is [%s] for [%s]' % ("short position limit", ccyLimitShort, self.__class__.__name__)
		...
		assert self.ccyLimitShort <= 0, '[%s] cannot be +ve for [%s]' % ("short position limit", self.__class__.__name__)
